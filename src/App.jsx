...
function activeCategories(exercises, removed = []) {
  const merged = [...CATEGORIES];
  exercises.forEach((e) => {
    if (e.category && !merged.includes(e.category)) merged.push(e.category);
  });
  return merged.filter((c) => !removed.includes(c) || exercises.some((e) => e.category === c));
}

function categoryUsageCounts(exercises) {
  const counts = {};
  exercises.forEach((e) => {
    ...
    <div className="rounded-lg p-2.5 w-full min-w-0" style={{ background: "#211f18", border: "1px solid #2a2820" }}>
      ...
      <div className="grid grid-cols-2 gap-2 min-w-0">
        <div className="min-w-0">
          <div className="mb-0.5" style={{ color: "#5c584c", fontSize: 9 }}>
            كجم
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div
        ...
        className="w-full max-w-md rounded-t-3xl p-4 pb-6 overflow-y-auto overflow-x-hidden box-border"
        style={{ background: "#1c1b15", border: "1px solid #2a2820", borderBottom: "none", maxHeight: "85vh", boxShadow: "0 -12px 32px rgba(0,0,0,0.4)" }}
      >
          ...
          {sets.map((row, idx) => (
            <div
              ...
              style={{
                ...
                maxHeight: leavingId === row.id ? 0 : 200,
                opacity: leavingId === row.id ? 0 : 1,
                animation: "setRowIn 0.2s ease",
              }}
            >
              <SetRowEditor
                idx={idx}
...
function ConfirmDialog({ title, message, confirmLabel = "حذف", cancelLabel = "إلغاء", onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 flex items-center justify-center px-6" style={{ background: "rgba(0,0,0,0.7)", zIndex: 60 }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl p-5" style={{ background: "#1c1b15", border: "1px solid #2a2820", boxShadow: "0 12px 32px rgba(0,0,0,0.5)" }}>
...
// ---------- Category picker: existing categories as chips, plus "+ new" to type a custom one ----------
function CategoryPicker({ categories, value, onChange, usageCounts, onRemoveCategory }) {
  const [adding, setAdding] = useState(false);
  const [newCat, setNewCat] = useState("");
  return (
    <div className="flex gap-2 flex-wrap items-center mb-5 min-w-0">
      {categories.map((c) => {
        const unused = onRemoveCategory && (!usageCounts || !usageCounts[c]);
        return (
          <div key={c} className="relative" style={{ paddingTop: unused ? 6 : 0, paddingInlineStart: unused ? 6 : 0 }}>
            <button
              type="button"
              onClick={() => onChange(c)}
              className="px-3 py-1.5 rounded-full text-xs"
              style={{
                background: value === c ? "#2e5090" : "#211f18",
                color: value === c ? "#f3efe2" : "#9b9686",
                border: "1px solid " + (value === c ? "#2e5090" : "#2a2820"),
              }}
            >
              {c}
            </button>
      ...
      {!adding ? (
        <button
          ...
          className="px-3 py-1.5 rounded-full text-xs"
          style={{ background: "#211f18", color: "#9b9686", border: "1.5px dashed #3a362c" }}
        >
          + قسم جديد
        </button>
      ) : (
        <div className="flex items-center gap-1.5">
          <input
            ...
            style={{ background: "#211f18", color: "#f3efe2", border: "1px solid #2a2820", width: 96 }}
          />
          <button
            type="button"
            onClick={() => {
...
function EditExerciseSheet({ exercise, categories, usageCounts, onRemoveCategory, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div
        ...
        style={{ background: "#1c1b15", border: "1px solid #2a2820", borderBottom: "none", boxShadow: "0 -12px 32px rgba(0,0,0,0.4)" }}
      >
        ...
        />
        <CategoryPicker categories={categories} value={cat} onChange={setCat} usageCounts={usageCounts} onRemoveCategory={onRemoveCategory} />
        <button
          type="button"
          disabled={!name.trim()}
          onClick={() => {
...
// ---------- Exercises Tab ----------
function ExercisesTab({ exercises, sessions, onLog, onAddExercise, favorites, onToggleFavorite, onUpdateExercise, onDeleteExercise, onUpdateSessionSets, onDeleteSession, removedCategories, onRemoveCategory }) {
  ...
  const categories = activeCategories(exercises, removedCategories);
  const usageCounts = categoryUsageCounts(exercises);
      ...
      {!addOpen ? (
        ...
        <div className="rounded-2xl p-4 mt-2" style={{ background: "#1c1b15", border: "1px solid #2a2820" }}>
          ...
          <div className="mb-3 min-w-0">
            <CategoryPicker categories={categories} value={newCat} onChange={setNewCat} usageCounts={usageCounts} onRemoveCategory={onRemoveCategory} />
          </div>
          <div className="flex gap-2">
            <button
              type="button"
...
function MultiSetLineChart({ data, maxSets, metricPrefix, unit, title }) {
  return (
    <div className="mb-4">
      ...
      <div className="flex gap-3 flex-wrap mt-2">
        {Array.from({ length: maxSets }).map((_, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: SET_COLORS[i % SET_COLORS.length] }} />
            <span style={{ color: "#9b9686", fontSize: 11, fontFamily: "'Tajawal', sans-serif" }}>المجموعة {i + 1}</span>
          </div>
        ))}
      </div>
    </div>
...
function ProgressTab({ exercises, sessions, onDelete, removedCategories }) {
  ...
  // group all sets by date (same-day sessions concatenate in order), then build one
  // independent series per "set position" — set 1's own line, set 2's own line, etc. —
  // exactly like the reference chart: each set stands alone, never averaged or capped.
  const byDate = {};
  const dateOrder = [];
  exSessions.forEach((s) => {
    ...
    byDate[s.date].push(...s.sets);
  });
  dateOrder.sort();

  const maxSets = dateOrder.reduce((m, d) => Math.max(m, byDate[d].length), 0);
  const dateChartData = dateOrder.map((d) => {
    const row = { date: d, label: fmtDate(d) };
    byDate[d].forEach((st, i) => {
      row[`w${i + 1}`] = st.weight;
      row[`r${i + 1}`] = st.reps;
    });
    return row;
  ...
  const allSets = [];
  exSessions.forEach((s) => s.sets.forEach((st) => allSets.push(st)));
  const bestWeightPoint = allSets.reduce((a, b) => (b.weight > a.weight ? b : a));
  const bestRepsPoint = allSets.reduce((a, b) => (b.reps > a.reps ? b : a));
  const weightDelta = allSets[allSets.length - 1].weight - allSets[0].weight;
    ...
    <div className="px-4 pb-8">
      {grouped.map((g) => (
        <div key={g.cat} className="mb-3">
          <div className="font-bold mb-1.5 px-1" style={{ color: "#5c584c", letterSpacing: 1, fontSize: 11 }}>
            {g.cat.toUpperCase()}
          </div>
      ...
      <div className="rounded-2xl p-4 mt-3 mb-4" style={{ background: "#1c1b15", border: "1px solid #2a2820" }}>
        ...
        <MultiSetLineChart data={dateChartData} maxSets={maxSets} metricPrefix="w" unit=" kg" title="تطور الوزن" />
        <MultiSetLineChart data={dateChartData} maxSets={maxSets} metricPrefix="r" unit="" title="تطور التكرارات" />
      </div>
        ...
        {exSessions
          ...
          .map((s, i) => (
            <div
              ...
              style={{
                background: "#1c1b15",
                borderTop: i > 0 ? "1px solid #2a2820" : "none",
                overflow: "hidden",
                transition: "max-height 0.16s ease, opacity 0.16s ease, padding 0.16s ease",
                maxHeight: leavingSessionId === s.id ? 0 : 60,
                opacity: leavingSessionId === s.id ? 0 : 1,
                paddingTop: leavingSessionId === s.id ? 0 : undefined,
                paddingBottom: leavingSessionId === s.id ? 0 : undefined,
              }}
            >
                ...
                {confirmId === s.id ? (
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      style={{ background: "#c1272d", color: "#f3efe2", fontSize: 11 }}
                    >
                      تأكيد الحذف
                    </button>
                    <button type="button" onClick={() => setConfirmId(null)} style={{ color: "#5c584c", fontSize: 11 }}>
                      إلغاء
                    </button>
                  </div>
                ) : (
...
export default function GymTracker() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (parsed.removedCategories) setRemovedCategories(parsed.removedCategories);
      }
    } catch (e) {
  ...
  useEffect(() => {
    if (!loaded) return;
    try {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ exercises, sessions, lastTimerDuration, favorites, removedCategories }));
    } catch (e) {
      // ignore save errors
    }
  }, [exercises, sessions, lastTimerDuration, favorites, removedCategories, loaded]);
  ...
  const playAlarm = useCallback(() => {
    ...
    try {
      if ("Notification" in window && Notification.permission === "granted") {
        new Notification("⏱️ خلصت الراحة!", { body: "يلا على المجموعة الجاية 💪", silent: true, tag: "gym-rest-timer" });
      }
    } catch (e) {
      // notifications unavailable, ignore — the sound/vibration above still cover it
    }
    if (!ctx) return;
  ...
  const handleRemoveCategory = (cat) => setRemovedCategories((prev) => (prev.includes(cat) ? prev : [...prev, cat]));
  ...
  return (
    <div dir="rtl" style={{ background: "#16150f", minHeight: "100vh", fontFamily: "'Tajawal', sans-serif" }}>
      ...
      <div className="pt-5" style={{ minHeight: "60vh" }}>
        ...
        {tab === "progress" && <ProgressTab exercises={exercises} sessions={sessions} onDelete={handleDeleteSession} removedCategories={removedCategories} />}
        {tab === "timer" && (
          <RestTimer
